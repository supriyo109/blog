...
const RecentBlog = () => {
    ...
    useEffect(() => {
        const getAllPublsihedBlogs = async () => {
            try {
                const res = await axios.get(`https://blog-2-s756.onrender.com/api/v1/blog/get-published-blogs`, { withCredentials: true })
                if (res.data.success) {
    ...
    return (
        <div className='bg-gray-100 dark:bg-gray-800 pb-10'>
            ...
            <div className='max-w-7xl mx-auto inline md:flex gap-6'>
                <div>
                ...
                <div className='bg-white dark:bg-gray-700 md:w-[350px] p-5 rounded-md mt-10'>
                    <h1 className='text-2xl font-semibold'>Popular categories</h1>
                    <div className='my-5 flex flex-wrap gap-3'>
                        {
                            tags.map((item, index) => {
                                return <Badge onClick={() => navigate(`/search?q=${item.category}`)} key={index} className="h-10 text-xl cursor-pointer">{item.category}</Badge>
                            })
                        }
                    </div>
