...
const CommentBox = ({ selectedBlog }) => {
    ...
    useEffect(() => {
        const getAllCommentsOfBlog = async () => {
            try {
                const res = await axios.get(`https://blog-5-qvj7.onrender.com/api/v1/comment/${selectedBlog._id}/comment/all`)
    ...
    const commentHandler = async () => {
        try {
            const res = await axios.post(`https://blog-5-qvj7.onrender.com/api/v1/comment/${selectedBlog._id}/create`, { content }, {
                headers: {
    ...
    const deleteComment = async (commentId) => {
        try {
            const res = await axios.delete(`https://blog-5-qvj7.onrender.com/api/v1/comment/${commentId}/delete`, {
                withCredentials: true
            })
            if (res.data.success) {
    ...
    const editCommentHandler = async (commentId) => {
        try {
            const res = await axios.put(
                `https://blog-5-qvj7.onrender.com/api/v1/comment/${commentId}/edit`,
     ...
     const likeCommentHandler = async (commentId) => {
         try {
             const res = await axios.get(
                 `https://blog-5-qvj7.onrender.com/api/v1/comment/${commentId}/like`,
                 {
                     withCredentials: true,
                 }
             );
                            ...
                            return <div key={index} className='mb-4'>
                                <div className='flex items-center justify-between'>
                                    <div className='flex gap-3 items-start'>
                                        <Avatar>                                    
                                            {item?.userId && item.userId.photo ? (
                                                <AvatarImage src={item.userId.photo} />
                                            ) : (
                                        ...
                                        <div className='mb-2 space-y-1 md:w-[400px]'>
                                            <h1 className='font-semibold'>{item?.userId?.firstname|| "Unknown"} {item?.userId?.lastname|| "Unknown"} <span className='text-sm ml-2 font-light'>
                                                {
                                                    item.updatedAt==item.createdAt?
                                                new Date(item.updatedAt).toLocaleDateString():
                                                 `Edited on ${new Date(item.updatedAt).toLocaleDateString()}`}
                                                </span></h1>
                                    ...
                                    {
                                        user?._id && item?.userId?._id && user._id === item.userId._id ? (<DropdownMenu>
